...
def shell_sorting(arr, seq, name):
    ...
    # calcula quantos grupos havera no array
    k_lim = len(arr_c) % seq[seq_i] 
    if len(arr_c) % seq[seq_i] == 0:
        k_lim = seq[seq_i]
    ...
    while seq_i >= 0:
        ...
        for k in range(k_lim + 1):
            ...
            # loops do insertion_sort
            while k + (j * seq[seq_i]) < len(arr_c):
                key = arr_c[k + j * seq[seq_i]]
                i = k + (j - 1) * seq[seq_i]

                while i >= 0 and arr_c[i] > key:
                    arr_c[i + seq[seq_i]] = arr_c[i]
                    i -= seq[seq_i]
                    moves += 1;
                
                arr_c[i + seq[seq_i]] = key;
                j += 1

        print_array(arr_c, f"INCR={seq[seq_i]}")
        seq_i -= 1
